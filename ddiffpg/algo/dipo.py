...
@dataclass
class AgentDIPO(ActorCriticBase):
    ...
    def update_net(self, memory):
        ...
        for i in range(self.cfg.algo.update_times):
            obs, action, target_action, reward, next_obs, done = memory.sample_batch(self.cfg.algo.batch_size)
            # intrinsic reward
            ...
            memory.update_target_action(new_action)
            actor_loss, actor_grad_norm = self.update_actor(obs, new_action)
