...
class DenoiseDiffusion:
    ...
    def origin_q_sample(self, x0: torch.Tensor, t: torch.Tensor, base_t=None):
        # Get batch size
        batch_size = x0.shape[0]
        T = torch.randint(0, self.n_steps, (batch_size,), device=x0.device, dtype=torch.long)
        ...
        if(base_t is not None):
            ...
            xT = (1 - alpha_T)*x0 + (alpha_T*xT)
    
        alpha = alpha.view(-1,1,1,1)
        xt = (1 - alpha)*x0 + (alpha*xT)

        return xt

    def origin_p_sample(self, xt: torch.Tensor, t: torch.Tensor):
        alpha_t = t / self.n_steps
        alpha_t_next = (t - 1)/ self.n_steps

        S_theta = self.eps_model(xt, t)

        next_x = xt - (1/self.n_steps *(xt - S_theta))

        return next_x


    def denoiseing_loss(self, x0: torch.Tensor, noise: Optional[torch.Tensor] = None):
        """
        #### loss targeting denoising rather than generative diffusion objective
        Source: https://arxiv.org/pdf/2305.04457.pdf
        uses the charbonnier loss: sqrt(mse(~x0,x0)+eps^2) where eps=0.001
        """
        # Get batch size
        batch_size = x0.shape[0]
        # Get random $t$ for each sample in the batch
    ...
    #TODO
    def adv_loss(self, victim_caption, victim_alphas, target_caption, x0: torch.Tensor, noise: Optional[torch.Tensor] = None):
      """
      #### Simplified Loss
      ...
      # MSE loss
      mse_loss = F.mse_loss(noise, eps_theta)
      
      # For each batch element, sample a certain percentage number of timesteps from the model
      number_of_t_samples = int(self.n_steps*0.2)
      t_sequence = torch.randint(0, self.n_steps, (batch_size,number_of_t_samples), device=x0.device, dtype=torch.long)
      #TODO check if batch t's work here
      xt_sequence = self.q_sample(x0, t_sequence)
      #generate predicted noise
      eps_theta_sequence = self.eps_model(xt_sequence, t_sequence)

      target_length = len(target_caption)
      input_length = number_of_t_samples
      #ctc_loss = torch.nn.CTCLoss(eps_theta_sequence,target_caption,input_length,target_length)
      #scores = pack_padded_sequence(scores, decode_lengths, batch_first=True).data
      #targets = pack_padded_sequence(targets, decode_lengths, batch_first=True).data
      # Calculate caption loss
      cap_loss = criterion(victim_caption, target_caption)
      # Add doubly stochastic attention regularization
      #TODO set this up, see if it works/helps/is needed
      alpha_c = 1.
      cap_loss += alpha_c * ((1. - alphas.sum(dim=1)) ** 2).mean()

      return mse_loss + cap_loss#ctc_loss
